...
Write-Verbose "Creating New OSDCloud WinRE Template to enable wireless networking support for Billy's desktop..." -Verbose
New-OSDCloudTemplate -Name OSDCloud-BillyPCs -WinRE
...
$WorkspacePath = "C:\OSDCloud\BillyDesktop"
...
#################################
# Download Billy Desktop motherboard drivers
#################################

Write-Host
Write-Verbose "Processing: Billy's Desktop motherboard drivers" -Verbose
Write-Host

$billyDrivers = Invoke-WebRequest("")
Invoke-Expression $($BillyDrivers.Content)
...
Write-Host
Write-Verbose "Processing: Integrating Billy's-Desktop motherboard drivers" -Verbose
Write-Host
...
Write-Host
Rename-Item -Path "$OSDCloudISOPath\OSDCloud.iso" -NewName "C:\OSDCloud\Gigabyte-AERO-AM4\OSDCloud-Billy-Desktop.iso" -Force
Rename-Item -Path "$OSDCloudISOPath\OSDCloud_NoPrompt.iso" -NewName "C:\OSDCloud\Gigabyte-AERO-AM4\OSDCloud-Billy-Desktop_NoPrompt.iso" -Force
...
Copy-Item -Path $ISO2 -Destination $OSDISODestinatio
